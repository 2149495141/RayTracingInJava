...
import RTExtended.HitTable.HitTable;
...
public class Render {
    ...
    void windowRenderBySPP(HitTable scene){
        JFrame window = new JFrame("Ray tracing in Java"){
            @Override
            public void paint(Graphics g) {
                super.paint(g);
                ...
                for (int y=0; y < image_height; ++y){
                    ...
                    for (int x=0; x < image_width; ++x){
                        ...
                        g.setColor(writeAwtColor(pixelColor, spp));
                        g.drawRect(x,y,1,1);
                    }
                    System.out.println("剩余行数:" + (image_height-y) + " | " + "耗时:" + (double)(System.currentTimeMillis()-start_time)/1000+"s");
                }
            }
        };
        window.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
