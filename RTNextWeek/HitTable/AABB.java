...
public class AABB {
    ...
    AABB add_offset(Vec3 offset) {
        return new AABB(x.add_offset(offset.x), y.add_offset(offset.y), z.add_offset(offset.z));
    }

    boolean less_than(AABB box_b, int axis){
        return axis(axis).min() < box_b.axis(axis).min();
    }

    boolean greater_than(AABB box_b, int axis){
        return axis(axis).min() > box_b.axis(axis).min();
    }
